...
class LinearNudgingAlg:
    ...
    def init_sim(self, ev_type, pp_type, mu_value, relax_time, bound_value, matrix_estimates, loop_limit, case_type):
        ...
        ev_graph = EigenvaluesGraph(ev_type, pp_type, case_type, [-bound_value, bound_value], loop_limit)
        line_graph = LineGraph(ev_type, pp_type, [-bound_value, bound_value], loop_limit, case_type)
        trace_det_graph = TraceDetGraph(ev_type, pp_type, [-bound_value, bound_value], loop_limit, case_type)
        threshold = 1e-5 # Values below this threshold are considered bad, while values above are considered good
        ...
        while i < loop_limit:
            ...
            U = np.array([[S[0, 2] - S[0, 0], S[0, 3] - S[0, 1]]])  # Error terms: U = [u, v] = [xt - x, yt - y]

            # Run the algorithm
            S_lists, U_lists, param_estimates, update_times, param_errors, avg_param_errors = self.nuding_algorithm(A, At, t, dt, T_R, param_estimates, update_times, S, M, U, case_type)
            # S_lists, U_lists, param_estimates, update_times, param_errors, avg_param_errors = self.nudging_alg_main_diag(A, At, t, dt, T_R, param_estimates, update_times, S, M, U, case_type)
            # S_lists, U_lists, param_estimates, update_times, param_errors, avg_param_errors = self.nudging_alg_anti_diag(A, At, t, dt, T_R, param_estimates, update_times, S, M, U, case_type)
            trace_det_graph.organize_data(A, param_estimates, avg_param_errors, trace_A, det_A, case_type)
            line_graph.organize_data(t, update_times, A, param_estimates, param_errors, avg_param_errors, i, threshold, S_lists, U_lists)
            ev_graph.organize_data(A, param_estimates, avg_param_errors, case_type)
    ...
    def nuding_algorithm(self, A, At, t, dt, T_R, param_estimates, update_times, S, M, U, case_type):
        ...
        a11_abs_err, a12_abs_err = [a11_estimates[0] - a11], [a12_estimates[0] - a12]
        a21_abs_err, a22_abs_err = [a21_estimates[0] - a21], [a22_estimates[0] - a22]

        a11_rel_err, a12_rel_err = [abs(a11_abs_err[0] / a11)],  [abs(a12_abs_err[0] / a12)]
        a21_rel_err, a22_rel_err  =[abs(a21_abs_err[0] / a21)], [abs(a22_abs_err[0] / a22)]
        ...
        for i in range(len(t) - 1):
            ...
            #Calculate and log parameter error
            param_1_abs_err.append(param_1_estimates[-1] - param_1)
            param_2_abs_err.append(param_2_estimates[-1] - param_2)
            param_1_rel_err.append(abs(param_1_abs_err[-1] / param_1))
            param_2_rel_err.append(abs(param_2_abs_err[-1] / param_2))
        ...
        return S_lists, U_lists, param_estimates, update_times, param_errors, avg_param_errors
